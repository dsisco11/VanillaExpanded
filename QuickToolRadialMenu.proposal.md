...
Place Light before available tool-category entries when it has a supported candidate, making it the screen-up wedge. Omit its wedge when no candidate exists. Cache and refresh its candidate with the other entries, including changes to held/offhand items, active-hotbar selection, and light-selection inputs. Changing only the displayed light item keeps the current geometry; adding or removing Light changes the wedge count. Future virtual entries can supply their own selection policy through the same small entry/provider contract; no public plugin framework or additional special entries are required now.
...
- Light source occupies the screen-up wedge before available tools when its shared selector finds a supported candidate and disappears when unavailable.

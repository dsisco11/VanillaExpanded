...
Review status: The action-time ItemStack reference lookup revision (2026-09-23) supersedes both the original server-owned design and the later callback-driven client session design. Phase 4 implementation is retained in commit `e0389e6`; its previously linked review files are absent from this checkout. A later user amendment changes the outer ring to [available-entry construction](AvailableEntryLayout.md). Candidate, input, and installed primitive evidence remain applicable. Real multiplayer behavior remains unverified.
...
| D7 | Use the named category table below as canonical provider order, clockwise from 12 o'clock. | A later user amendment includes only available candidates in the ring. Availability changes wedge count and positions; table order remains deterministic. |
...
The tool-category provider set covers the installed `EnumTool` categories explicitly. The virtual-entry amendment appends Light source after the 33 named tool categories in provider order. The ring contains only providers with an available candidate, so N ranges from 0 to 34. Mapping uses stable names rather than enum iteration order or translated labels. For N greater than zero, available wedge i has clockwise center angle `i * 360/N` from screen-up and boundaries half a wedge to either side. N=0 renders only the center. The center uses the separate identifier `unequip`.
...
A modded item using an existing category uses that category's provider and appears when selected as its candidate. Unknown numeric enum values are unsupported. A future supported-category extension requires an explicit versioned provider-order change. Inventory discovery changes the current available set and therefore the rendered wedge count, but does not change the supported provider set. Readability of the largest possible ring remains a runtime acceptance case.
...
The client session retains original and current ItemStack references, complete saved-stack snapshots, initially-empty-hand state, the original active hotbar position, and the current quick-tool's preferred home address. Quantities and selected entry are validation metadata. Remove expected displaced-item locations. Search eligible inventory for each exact reference on menu open and before switch/restore; each required object must occur once. Resolve source slots from that search and validate the home as a destination independently. A moved original stack remains usable wherever found; a current quick-tool outside the recorded hand ends the session. If both tracked objects are recreated, rebind only unique complete-stack and quantity matches. A lone replacement, ambiguity, changed whole-stack quantity, or unusable item ends restoration. No persistent item ID is introduced.

Native accepted updates and rejected-request corrections can change the client inventory and replace stack objects. Their arrival does not classify a quick-tool request. A paired recreation may preserve restoration through unique complete-stack matches; a lone replacement or ambiguous match cannot establish continuity. Verify this behavior in multiplayer acceptance. The inventory subscriptions and topology tick described above belong to candidate-cache maintenance; they do not introduce equipment-session callback state.
...
The shader effects and deterministic geometry remain exactly the approved design. Changes to available wedge count regenerate the mesh; candidate-item, hover, selection and animation changes do not. Scale changes may also retessellate to maintain visual tolerance. No profiling, timing comparison, or benchmark gate is added.
...
| Later live/runtime | Actual click-through/camera/focus behavior, shader reload, GUI scaling/readability across available wedge counts, item rendering, world lifecycle, native bag persistence and multiplayer sync. |
...
| --- | --- | --- | --- |
| Resolve D1-D7 | Proposal Candidate Discovery, Equipment, Inventory Changes, Decisions; checklist Implementation decisions | Preserve best-per-category, worn-first correction, A restoration and deterministic provider order; the later amendment controls available wedge count | Decision table and canonical map above; historical investigation audit applies to its original scope |
...
The earlier [second review](SecondReview.md) and [completion audit](CompletionAudit.md) apply to the original investigation. Earlier Phase 4 and Phase 5 review-record links did not resolve in the retained checkout. The [available-entry amendment](AvailableEntryLayout.md) records the current layout behavior and fresh verification. Native multiplayer evidence remains required.
